...
function replaceOnce(file, oldText, newText) {
  ...
  if (!text.includes(oldText)) {
    throw new Error(`Patch marker not found in ${file}: ${oldText.slice(0, 90)}`);
  }
  write(file, text.replace(oldText, newText));
}

function ensureVendorDir() {
  fs.mkdirSync(path.join("public", "vendor"), { recursive: true });
  fs.copyFileSync(
    path.join("node_modules", "three", "build", "three.module.js"),
    path.join("public", "vendor", "three.module.js")
  );
}
...
replaceOnce("server.js", "    bot.y = 0.85;", "    bot.y = 0.08;");
...
.speed-lines {
  ...
  background:
    radial-gradient(circle at 50% 54%, transparent 0 25%, rgba(255, 255, 255, 0.08) 26%, transparent 42%),
    repeating-conic-gradient(
      from 0deg at 50% 55%,
      transparent 0deg 11deg,
      rgba(126, 184, 255, 0.18) 12deg 13deg,
...
replaceOnce("public/app.js", "        mesh.target.set(entry.x, entry.y || 0.85, entry.z);", "        mesh.target.set(entry.x, entry.y ?? 0.08, entry.z);");
replaceOnce("public/app.js", "          y: player.y || 0.08,", "          y: player.y ?? 0.08,");
replaceOnce("public/app.js", "      return entry ? { x: entry.x, y: entry.y || 0.08, z: entry.z } : null;", "      return entry ? { x: entry.x, y: entry.y ?? 0.08, z: entry.z } : null;");
...
replaceOnce(
  ...
  `    function updateCamera(dt) {
      ...
      const back = 13 + speedPull * 7;
      const height = 7.5 + speedPull * 2.5;
